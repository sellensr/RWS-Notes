...
float v[] = {0, 0, 0, 0, 0, 0};  // voltage values
float x[] = {0, 0, 0, 0, 0, 0};  // actual outputs like temperature
...
void loop() {
  ...
  for(int i = 0;i < NCH;i++){ 
    v[i] = (as[i]+0.5)/1024 * 5.000;   // convert to voltage
    x[i] = v[i];                       // voltage by default
  }
